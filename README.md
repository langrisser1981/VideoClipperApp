...
5. **輸出成品**：確認好所有要剪的片段後，按「Export…」按鈕或 `Cmd+E`。程式會用 `AVMutableComposition` 把「沒被標記」的片段接起來，輸出成 `原檔名_年月日時分秒.mp4`（例如 `vacation_20260821153000.mp4`），存在原影片的同一個目錄。畫面下方會顯示匯出進度與完成訊息；如果還沒選影片或還沒標記任何區段，也會直接顯示對應的提示文字，而不是靜默無反應。
6. **標記自動記錄**：每次按「Export…」/`Cmd+E` 輸出前，目前的標記都會存成同目錄下的 `原檔名.clipmarks.json`。之後不管是關掉重開還是重新選擇同一部影片，只要那個標記檔還在，程式會自動載入之前的標記，不用重新標一次。
